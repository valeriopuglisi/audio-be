...
def asr_evaluate_metric_with_model_on_commonvoice(task, dataset, model, metrics, n_test):
    ...
    for i, row in enumerate(test_table.iterrows()):
        print("Benchmarking: {}/{}".format(i, tot_sample))
        ...
        try:
            ...
            if os.path.isfile(audio_path):
                ...
                if prediction is not None and prediction != "" and reference is not None and reference != "":
                    ...
                    print("len(predictions):{}, len(references):{}".format(len(predictions), len(references)))
                    test_done += 1
            else:
